# Phase E.3: Persistent Offline-First Architecture — Walkthrough
...
### Task C: Deep Sync Badge + Bulk Priority
...
**What changed:**
1. **New "Sync" column** in the item table header (between Status and Relists)
2. **Deep/Shallow badge** in each row — green "Deep" if `detail_hydrated_at` is set, amber "Shallow" otherwise
3. **Bulk sync priority** — within each status group, shallow items are now processed before already deep-synced items
