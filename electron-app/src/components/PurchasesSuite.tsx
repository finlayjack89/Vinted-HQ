/**
 * Purchases Suite — Bought orders dashboard with local persistence and enrichment
 *
 * Features:
 *  1. Status tabs: All / In Progress / Completed / Cancelled
 *  2. Background conversation enrichment (seller username, item_id)
 *  3. Local persistence in bought_orders SQLite table
 *  4. Listing price from wardrobe cross-reference
 *  5. Item ID display on cards
 *  6. Read-only detail modal on click
 *  7. Fixed card outline UI glitch
 */
...
export default function PurchasesSuite() {
  const [items, setItems] = useState<PurchaseDisplayItem[]>([]);
  ...
  const apiToDisplay = (apiItem: VintedSoldItem, cached?: BoughtOrderRow | null): PurchaseDisplayItem => ({
    ...
    seller_username: cached?.seller_username ?? null,
    listing_price: cached?.listing_price ?? null,
    enriched: !!cached?.enriched_at,
  });

  /** Convert local DB row -> display item */
  const cachedToDisplay = (row: BoughtOrderRow): PurchaseDisplayItem => ({
  ...
  const loadPurchases = useCallback(async (status: StatusKey, pg: number) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const result: BridgeResult = await window.vinted.getPurchasesApi(status, pg, perPage);
      ...
      if (!result.ok) {
        ...
        try {
          const saved = await window.vinted.getSavedBoughtOrders(status === 'all' ? undefined : status);
      ...
      let savedMap = new Map<number, BoughtOrderRow>();
      try {
        const saved = await window.vinted.getSavedBoughtOrders();
      ...
      for (const api of rawItems) {
        ...
        window.vinted.upsertBoughtOrder({
  ...
  /** Background enrichment — fetches conversation detail for each unenriched purchase */
  const enrichItems = async (unenriched: PurchaseDisplayItem[], signal: AbortSignal) => {
    for (const item of unenriched) {
      ...
      try {
        ...
        const sellerUsername = (oppositeUser?.login as string) ?? null;
        ...
        // Update local DB
        await window.vinted.upsertBoughtOrder({
        ...
        setItems(prev => prev.map(p =>
          p.transaction_id === item.transaction_id
            ? { ...p, item_id: itemId, seller_username: sellerUsername, photo_url: photoUrl, listing_price: listingPrice, enriched: true }
            : p
        ));
  ...
  useEffect(() => {
    loadPurchases(statusTab, page);
    return () => enrichAbort.current?.abort();
  }, [statusTab, page, loadPurchases]);
  ...
  const handleCardClick = (item: PurchaseDisplayItem) => {
    setDetailModal(item);
  };
  ...
  return (
    <div style={{ padding: spacing['2xl'], display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: colors.textMuted, fontSize: font.size.base }}>
            Your bought orders and purchase history
            {enriching && <span style={{ marginLeft: 8, color: colors.primary, fontSize: font.size.xs }}>● Enriching…</span>}
          </p>
        </div>
        <button type="button" onClick={() => loadPurchases(statusTab, page)} disabled={loading}
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: font.weight.semibold, color: colors.error, fontSize: font.size.base, marginBottom: 2 }}>
              {error.code === 'DATADOME_CHALLENGE' || error.code === 'DD_CHALLENGE' ? 'Bot Challenge Detected'
                : error.code === 'SESSION_EXPIRED' ? 'Session Expired' : 'Failed to Load Purchases'}
            </div>
            <div style={{ color: colors.textSecondary, fontSize: font.size.sm }}>{error.message}</div>
          </div>
          <button type="button" onClick={() => loadPurchases(statusTab, page)} style={{ ...btnSecondary, ...btnSmall }}>Retry</button>
        </div>
      )}
      ...
      {!loading && items.length === 0 && !error && (
        <div style={{ ...glassPanel, padding: spacing['4xl'], textAlign: 'center' }}>
          ...
          <p style={{ margin: 0, color: colors.textMuted, fontSize: font.size.base, maxWidth: 360, marginLeft: 'auto', marginRight: 'auto' }}>
            When you buy items on Vinted, they'll appear here with full transaction details.
          </p>
        </div>
      )}
      ...
      {!loading && items.map(item => (
        <PurchaseItemCard key={item.transaction_id} item={item} cardStyle={cardStyle} cardHoverStyle={cardHoverStyle}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: `${spacing.sm}px ${spacing.xl}px` }}>
          ...
          {item.seller_username && (
            <div>
              ...
              <div style={{ ...valueStyle, color: colors.primary }}>@{item.seller_username}</div>
            </div>
          )}
          {item.item_id && (
            <div>
